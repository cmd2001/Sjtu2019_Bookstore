...
public:
    ...
    void reportFinance() {
        const int limit = (int)file.tellp() - 16;
        ifstream fin(fileName.c_str(), ios::binary);
        double x;
        while(fin.tellg() != limit) {
            fin.read(reinterpret_cast<char*> (&x), 8);
            if(x < 0) cout << "Outcome " << setprecision(2) << fixed << x << endl;
            else cout << "Income " << setprecision(2) << fixed << x << endl;
        }
        cout << "Sum = ", showAll();
    }
};
...
public:
    ...
    void addOperate(const User &user, const string &ope) {
        fout << "User: " << user.id << " Name:" << user.name << " privilege: " << user.type << " operation: " << ope << endl;
    }
    void reportAll() {
        ifstream fin(fileName.c_str());
        string t;
        while(getline(fin, t)) cout << t << endl;
    }
    void reportUser(const string &uid) {
        ifstream fin(fileName.c_str());
        string t, x;
        while(getline(fin, t)) {
            stringstream ss;
...
private:
    ...
    void work() {
        ...
        curUser = (User){"guest", "", "guest", 0};
        suStack.push(curUser);
        curUser = (User){"root", "sjtu", "root", 7};
...
#define checkSS() if(!ss.eof()) {invalid(); continue;}
            if(com == "su") {
                ...
                checkSS();
                if(s2 == "finance") {
                    if(curUser.type != 7) {invalid(); continue;}
                    fl.reportFinance();
                } else if(s2 == "employee") {
                    if(curUser.type != 7) {invalid(); continue;}
                    opeLog.reportAll();
                } else if(s2 == "myself") opeLog.reportUser(curUser.id);
            ...
            opeLog.addOperate(curUser, line);
            // debug << "==================================================" << endl;
        }
    }
public:
